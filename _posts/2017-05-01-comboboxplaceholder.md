...
## 解説
上記のサンプルでは、編集不可の`JComboBox`にアイテムが選択されていない場合はヒント(プレースホルダ文字列)を表示するリストセルレンダラーを設定しています。
...
- リストセルレンダラーは、`DefaultListCellRenderer#getListCellRendererComponent(...)`メソッドをオーバーライドし、引数の`value`が`null`の場合のみヒント文字列を表示するコンポーネントを返すよう設定
    - 注: 引数の`Object value`が`null`の場合ではなく、`int index`が`-1`の場合にヒント文字列を表示するように設定すると、マウスクリックなどで選択が変更できなくなる？
